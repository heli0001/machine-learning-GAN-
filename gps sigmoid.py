...
def discriminator(X, reuse=False):
    ...
    # fully connected
        ...
        logits = tf.layers.dense(flat, 1) # output of the final layer
...
gen_step = tf.train.RMSPropOptimizer(learning_rate=0.0001).minimize(gen_loss, var_list= gen_vars)
disc_step = tf.train.RMSPropOptimizer(learning_rate=0.0001).minimize(disc_loss, var_list= disc_vars)
...
#     X_batch = data_sample_tensor(batchsize, blocksize).reshape([batchsize, blocksize, 10,1])
...
for i in range(epoch):
    ...
    plt.tight_layout()
    plt.savefig('pics/gps_sigmoid_sample%d.png' % i)
    plt.close()
